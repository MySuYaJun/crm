...
<script type="text/javascript">
	...
	$(function(){
        ...
        $("#searchActivityText").keyup(function () {
            ...
            //var name=this.value;
            //var name=$(this).val();
            var name=$("#searchActivityText").val();
            var clueId='${clue.id}';
            //发送请求
            $.ajax({
		...
		$("#relationTBody").on("click","a",function(){
			...
			var clueId='${clue.id}';
			//在js中获取元素的属性值：
			//方式1：dom对象.属性名     只能获取元素固有属性，不能获取自定义属性
			//方式2：jquery对象.attr("属性名")  不但能获取元素固有属性，还能获取元素的自定义属性，但是不能获取那些值是true|false的固有属性
			//方式3：jquery对象.prop("属性名")  能够获取那些值是true|false的固有属性
					...
					<tbody id="relationTBody">
                        ...
                        <c:forEach items="${activityList}" var="act">
                            <tr id="tr_${act.id}">
                                ...
                                <td>${act.endDate}</td>
                                <td>${act.owner}</td>
                                <td><a activityId="${act.id}" href="javascript:void(0);"  style="text-decoration: none;"><span class="glyphicon glyphicon-remove"></span>解除关联</a></td>
                            </tr>
                        </c:forEach>
					</tbody>
				</table>
