...
<body>
	...
	<div style="position: relative; left: 40px; top: -50px;">
		...
		<c:forEach items="${stageList}" var="stage" varStatus="vs">
            <!--如果stage是最后两个阶段，则图标显示为thumbs-down-->
            <c:if test="${vs.count==fn:length(stageList)||vs.count==fn:length(stageList)-1}">
                <!--如果stage就是当前交易的阶段，则颜色显示为红色-->
                <c:if test="${tran.stage==stage.value}">
                    <span class="glyphicon glyphicon-thumbs-down mystage" data-toggle="popover" data-placement="bottom" data-content="${stage.value}" style="color: red;"></span>
                    -----------
                </c:if>
                <!--如果stage不是当前交易的阶段，则颜色显示为黑色-->
                <c:if test="${tran.stage!=stage.value}">
            ...
            <!--如果stage是倒数第三个阶段，则图标显示为thumbs-up-->
            <c:if test="${vs.count==fn:length(stageList)-2}">
                <!--如果stage就是当前交易阶段，则颜色显示显示为绿色-->
                <c:if test="${tran.stage==stage.value}">
                    <span class="glyphicon glyphicon-thumbs-up mystage" data-toggle="popover" data-placement="bottom" data-content="${stage.value}" style="color: #90F790;"></span>
                    -----------
                </c:if>
                <!--如果stage不是当前交易的阶段，则颜色显示为黑色-->
                <c:if test="${tran.stage!=stage.value}">
            ...
            <!--如果stage是普通阶段，则图标不确定，需要进一步判断-->
            <c:if test="${vs.count<fn:length(stageList)-2}">
                <!--如果当前交易阶段处在"成交"或者"成交"之前的阶段，分三种情况：-->
                <c:if test="${tran.orderNo<=stageList[fn:length(stageList)-3].orderNo}">
                    <!--如果stage就是交易当前的阶段，则图标显示为map-marker，颜色显示为绿色-->
                    <c:if test="${tran.stage==stage.value}">
                    ...
                    <!--如果stage处在交易当前阶段之前，则图标显示为ok-circle，颜色显示为绿色-->
                    ...
                    <!--如果stage处在交易当前阶段之后，则图标显示为record，颜色显示为黑色-->
                    <c:if test="${stage.orderNo>tran.orderNo}">
                        <span class="glyphicon glyphicon-record mystage" data-toggle="popover" data-placement="bottom" data-content="${stage.value}"></span>
                        -----------
                    </c:if>
                </c:if>
                <!--如果当前交易阶段处在"成交"之后，则继续需要记住失败状态，再分三种情况-->
                <c:if test="${tran.orderNo>stageList[fn:length(stageList)-3].orderNo}">
                    <!--如果stage就是失败之前最后一个非失败阶段，则图标显示为map-marker，颜色显示为黄色-->
                    ...
                    <!--如果stage处在失败之前最后一个非失败阶段之前，则图标显示为ok-circle，颜色显示为绿色-->
                    <c:if test="${stage.orderNo<theOrderNo}">
                        <span class="glyphicon glyphicon-ok-circle mystage" data-toggle="popover" data-placement="bottom" data-content="${stage.value}" style="color: #90F790;"></span>
                        -----------
                    </c:if>
                    <!--如果stage处在失败之前最后一个非失败阶段之后，则图标显示为record，颜色显示为黑色-->
                    <c:if test="${stage.orderNo>theOrderNo}">
                        <span class="glyphicon glyphicon-record mystage" data-toggle="popover" data-placement="bottom" data-content="${stage.value}"></span>
                        -----------
                    </c:if>
